...
public class MqttAndroidClient extends BroadcastReceiver implements
	...
	private void registerReceiver(BroadcastReceiver receiver) {
		...
		LocalBroadcastManager.getInstance(myContext).registerReceiver(receiver,
				filter);
	 ...
	 * <p>
	 * A convenience method, which will create a new {@link MqttMessage} object
	 * with a byte array payload, the specified QoS and retained, then publish
	 * it.
	 * </p>
	 * 
	 * @param topic
	 ...
	 *            service will be received using the QoS specified on the
	 *            subscription.
	 * @return token used to track and wait for the subscription to complete.
	...
	public IMqttToken subscribe(String topic, int qos, Object userContext,
		...
		IMqttToken token = new MqttTokenAndroid(this, userContext, callback,
				new String[] { topic });
		String activityToken = storeToken(token);
		Log.i("xionglu", "clientHandle:" + clientHandle);
		Log.i("xionglu", "activityToken:" + activityToken);
